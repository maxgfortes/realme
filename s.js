...
async function configurarBotaoSeguir() {
  ...
  // Se for o próprio perfil, esconder botão
...
function criarElementoDepoimento(depoData, autorData, depoId, targetUsername) {
  ...
  // Verificar se é o autor do depoimento (para mostrar botão de excluir)
  const currentUserJson = localStorage.getItem('usuarioLogado');
  const isAuthor = currentUserJson ? JSON.parse(currentUserJson).username === depoData.username : false;
  ...
  depoElement.innerHTML = `
    <div class="depoimento-header">
      ...
      ${isAuthor ? `<button class="delete-depo-btn" onclick="excluirDepoimento('${depoId}', '${targetUsername}')">
...
function aplicarBackgroundHeaderPhoto(dados) {
  if (dados.headerphoto) {
    ...
    console.log('✅ Background do headerphoto aplicado ao body');
  }
}
...
// ===================
// CSS MELHORADO PARA POSTS, STATUS, DEPOIMENTOS E LINKS
// ===================
const enhancedCSS = `
<style>
/* Status Indicators */
...
@media (prefers-color-scheme: dark) {
  .post-card, .depoimento-card, .depoimento-form, .user-link {
    background: #1a1a1a;
    border-color: #333;
    color: #fff;
  }
  
  .display-name, .post-content p, .depoimento-content p, .autor-nome {
    color: #fff;
  }
  
  .username-small, .post-time, .depo-time, .char-count {
    color: #aaa;
  }
...
.post-card, .depoimento-card, .depoimento-form {
...
.empty-depoimentos, .empty-links, .empty-posts {
