...
function setFooterLabelText(labelEl, type, date) {
    ...
    labelEl.appendChild(
        document.createTextNode(` ${formatTimeAgo(date)}`)
    );
}
...
export function renderMessages(messages, chatId, otherUserName) {
    ...
    messages.forEach(function(message, index) {
        ...
        if (reactionKeys.length > 0) {
            ...
            reactionBadge.textContent =
                reactionKeys.length > 1
                    ? `❤️ ${reactionKeys.length}`
                    : "❤️";
    ...
    if (wasNearBottom) {
        ...
        messagesList.scrollTop =
            prevScrollTop +
            (messagesList.scrollHeight - prevScrollHeight);
    }
